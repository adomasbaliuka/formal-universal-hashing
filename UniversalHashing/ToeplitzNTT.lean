...
`BitVec` family is universal₂ too. The transport goes through the bijection
`BitVec w ≃ (Fin w → ZMod 2)` given by `BitVec.toZMod2Fun`.

Note that perfect **Δ**-universality (`toeplitzHash.deltaUniversal2`) is deliberately
*not* restated here. `deltaUniversal2` is relative to an `AddCommGroup Output`, and the
instance `BitVec m` carries is arithmetic mod `2 ^ m` (from its `CommRing`), not
bitwise XOR. Those two groups are not isomorphic, so the Δ-statement does not transport
along the bijection: it would be a different — and unproven — claim about mod-`2 ^ m`
differences. Universality involves only collisions, so it transports without any group
structure.
-/
...
/-- `BitVec w` is a finite type, with `2 ^ w` elements. Mathlib does not supply this. -/
instance instFintypeBitVec (w : ℕ) : Fintype (BitVec w) :=
  Fintype.ofEquiv _ (bitVecEquivZMod2Fun w).symm
